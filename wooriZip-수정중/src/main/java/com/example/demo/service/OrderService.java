...
public class OrderService {
    ...
    public SpringDataJaxb.OrderDto getOrder(String email) {
        Users user = userRepository.findByEmail(email).orElseThrow(() -> new IllegalArgumentException("유저를 찾을 수 없습니다"));

        SpringDataJaxb.OrderDto orderDto = getExistingOrderDto(user);
        if (orderDto != null) {
            return orderDto;
        } else {
...
//                }
    ...
    // 주문 아이템 삭제
    public void removeOrderDetail(Long orderId, Long orderDetailId, String email) {
        Users user = userRepository.findByEmail(email).orElseThrow(() -> new IllegalArgumentException("유저를 찾을 수 없습니다"));
